...
static int  handle_int(long n, t_format *f)
{
	...
	if (!num)
		return (0);
	num_len = (int)ft_strlen(num) - (sign < 0);
	if (f->dot && f->precision == 0 && n == 0)
		num_len = 0;
	int	prefix_len = (sign < 0) || f->flag_plus || f->flag_space;
	pad_zero = f->precision - num_len;
	if (pad_zero < 0)
		pad_zero = 0;
	if (!f->dot && f->flag_zero && !f->flag_minus)
		pad_zero = max(pad_zero, f->width - (prefix_len + num_len));
	if (!f->flag_minus)
		ft_putnchar(' ', f->width - (prefix_len + num_len + pad_zero), &len);
	print_number_prefix(sign, f, &len);
	ft_putnchar('0', pad_zero, &len);
	if (!(f->dot && f->precision == 0 && n == 0))
		len += write(1, (sign < 0) ? num + 1 : num, num_len);
	if (f->flag_minus)
		ft_putnchar(' ', f->width - (prefix_len + num_len + pad_zero), &len);
	return (free(num), len);
}
